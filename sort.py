...
# p = Path(sys.argv[1])
# p = Path('/users/barts/desktop/bałagan')
def sort(p):
    # końcówki rozszerzeń
    ext_arc = ('.zip', '.gz', '.tar')
    ...
    for i in p.iterdir():
        ...
        if i.name.endswith(ext_arc):
            if i.name.endswith('.zip'):
                shutil.unpack_archive(i, p_arc/i.name.removesuffix('.zip'))
            elif i.name.endswith('.gz'):
                shutil.unpack_archive(i, p_arc/i.name.removesuffix('.gz'))
            elif i.name.endswith('.tar'):
                shutil.unpack_archive(i, p_arc/i.name.removesuffix('.tar'))
            os.remove(i)
        #audio
